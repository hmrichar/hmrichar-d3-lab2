...
const dataFile = await d3.csv(".data/miserables.csv"); // await is needed because d3.csv() returns a promise, so we need to wait for the data to load before we can use it
...
function convertToGraph(rows) {
    ...
    for (const row of rows) {
        groupMap.set(row.source, +row['source group']); // + converts the string to a number
