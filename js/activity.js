...
document.getElementById("uniqname").textContent = "hmrichar";
...
select.selectAll("option")
    ...
    .join("option")
    .attr("value", d => d)
    .text(d => d === "all" ? "All Airlines" : airlineName[d]);
...
function overed(event, d) {
    const incomingPaths = d.incoming.map(linkTuple => linkTuple.path);
    const outgoingPaths = d.outgoing.map(linkTuple => linkTuple.path);
    const incomingTexts = d.incoming.map(([source]) => source.text);
    const outgoingTexts = d.outgoing.map(([, target]) => target.text);

    link.style("mix-blend-mode", null).attr("stroke-opacity", 0.08).attr("stroke-width", 1);
    d3.select(this).attr("font-weight", "bold").attr("fill", "#111");

    d3.selectAll(incomingPaths).attr("stroke-opacity", 0.9).attr("stroke-width", 2.2).raise();
    d3.selectAll(outgoingPaths).attr("stroke-opacity", 0.9).attr("stroke-width", 2.2).raise();

    d3.selectAll(incomingTexts).attr("fill", "#d62728").attr("font-weight", "bold");
    d3.selectAll(outgoingTexts).attr("fill", "#2ca02c").attr("font-weight", "bold");
}

function outed(event, d) {
    link.style("mix-blend-mode", "multiply");
    link.attr("stroke-opacity", 0.35).attr("stroke-width", 1.2);
    d3.select(this).attr("font-weight", null).attr("fill", null);
    d3.selectAll(d.incoming.map(([source]) => source.text)).attr("fill", null).attr("font-weight", null);
    d3.selectAll(d.outgoing.map(([, target]) => target.text)).attr("fill", null).attr("font-weight", null);
}
